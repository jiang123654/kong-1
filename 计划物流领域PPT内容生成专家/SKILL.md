...
> 设计理念：基于审计模板的反向约束，生成"一次过审"的高质量PPT内容
...
graph TD
    ...
    B --> C[Skill 02: 大纲生成与逻辑分组]
    C --> D[Skill 03: 内容生成与模块设计]
    D --> E[Skill 04: 数据页总结与可视化]
    E --> F[Skill 05: 输出格式化与审核]
    F --> G[输出.MD文档]
```
...
| **Phase 2** | Skill 02 | 生成PPT大纲，逻辑分组，页面规划 | PPT大纲（含页面类型和核心结论） |
| **Phase 3** | Skill 03 | 逐页生成内容，设计模块布局 | 逐页PPT内容（含小标题+描述） |
| **Phase 4** | Skill 04 | 数据页总结描述，图表建议 | 数据页增强内容 |
| **Phase 5** | Skill 05 | 格式化输出，审计合规检查 | 最终.MD文档 |
...
```
[主题]_[日期]_PPT内容大纲.md
示例：2026年物流H1总结H2规划_20260625_PPT内容大纲.md
```
...
def execute(user_input):
    ...
    # Step 2: 调用Skill 02生成大纲
    outline = skill_02_outline_generator.create(parsed_data)
    
    # Step 3: 调用Skill 03生成内容
    content = skill_03_content_generator.generate(outline)
    
    # Step 4: 调用Skill 04处理数据页
    enhanced_content = skill_04_data_page_handler.enhance(content)
    
    # Step 5: 调用Skill 05格式化输出
    final_output = skill_05_output_formatter.format(enhanced_content)
...
| 输入技能 | 输出内容 | 接收技能 | 接收方式 |
|---------|---------|---------|---------|
| Skill 01 | 结构化信息素材库 | Skill 02 | 直接传递 |
| Skill 02 | PPT大纲（页面列表） | Skill 03 | 按页循环生成 |
| Skill 03 | 逐页内容（文本） | Skill 04 | 筛选数据页处理 |
| Skill 04 | 增强数据页内容 | Skill 05 | 合并输出 |
| Skill 05 | 最终.MD文档 | 用户 | 直接呈现 |
